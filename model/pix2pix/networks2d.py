...
class GANLoss(nn.Module):
    ...
    def __call__(self, prediction, target_is_real):
        ...
        if self.gan_mode in ['lsgan', 'vanilla']:
            target_tensor = self.get_target_tensor(prediction, target_is_real).to(prediction.device)
...
class NLayerDiscriminator3D(nn.Module):
    ...
    def __init__(self, input_nc, ndf=64, n_layers=3, norm="batch"):
        ...
        kw = 4
        padw = 1
        sequence = [nn.Conv3d(input_nc, ndf, kernel_size=kw, stride=2, padding=padw), nn.LeakyReLU(0.2)]
        nf_mult = 1
        nf_mult_prev = 1
        for n in range(1, n_layers):  # gradually increase the number of filters
            nf_mult_prev = nf_mult
...
if __name__ == '__main__':
    model = NLayerDiscriminator3D(input_nc=1, ndf=32, n_layers=3)
    model.cuda()
    print(model(torch.rand((1, 1, 320, 320, 128)).cuda()).shape)