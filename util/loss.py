...
class CrossEntropyAndDice(nn.Module):
    def __init__(self, ignore_index, alpha=1.0):
        super(CrossEntropyAndDice, self).__init__()
        self.ignore_index = ignore_index
        self.ce = torch.nn.CrossEntropyLoss(ignore_index=ignore_index)
        self.dice = DiceLoss(mode='multiclass', ignore_index=ignore_index)
        self.alpha = alpha
    def forward(self, pred, target):
        return self.ce(pred, target) + self.alpha * self.dice(pred, target)
